...
def gp_interpolator(x, y, res=1000, Nparam=3, decouple_sfr=False):
    ...
    kernel = np.var(y) * (
        kernels.Matern32Kernel(np.median(y))
...
def gp_sklearn_interpolator(x, y, res=1000):
    try:
        ...
        raise ImportError(
            "sklearn not available. The `gp_sklearn` interpolator requires scikit-learn."
        ) from exc
...
def tuple_to_sfh(
    sfh_tuple,
    zval,
    interpolator="gp_george",
    decouple_sfr=False,
    decouple_sfr_time=10,
    sfr_tolerance=0.05,
    vb=False,
    cosmo=cosmo,
    res=1000,
    ...
    for const_vals in SFH_constraint_percentiles:
        ...
        if (delta_t > time_quantiles[-2]) & (delta_t > 0.9):
            mass_quantiles = np.insert(
                mass_quantiles,
                -1,
                [const_vals],
            )
            time_quantiles = np.insert(
                time_quantiles,
                -1,
                [delta_t],
            )
            ...
            delta_m = 1 - (
                (cosmo.age(zval).value * 1e9) * (1 - const_vals) * (10 ** sfh_tuple[1])
            ) / (10 ** sfh_tuple[0])
    ...
    if interpolator == "gp_george":
        time_arr_interp, mass_arr_interp = gp_interpolator(
            time_quantiles,
            mass_quantiles,
            Nparam=int(Nparam),
            decouple_sfr=decouple_sfr,
            res=res,
        )
    elif interpolator == "gp_sklearn":
        time_arr_interp, mass_arr_interp = gp_sklearn_interpolator(
            time_quantiles, mass_quantiles
        )
    elif interpolator == "linear":
        time_arr_interp, mass_arr_interp = linear_interpolator(
            time_quantiles, mass_quantiles
        )
    elif interpolator == "pchip":
        time_arr_interp, mass_arr_interp = Pchip_interpolator(
            time_quantiles, mass_quantiles
        )
    else:
        raise Exception(
            "specified interpolator does not exist: {}. \n use one of the following: gp_george, gp_sklearn, linear, and pchip ".format(
                interpolator
            )
        )
    ...
    sfr_decouple_time_index = np.argmin(
        np.abs(time_arr_interp * cosmo.age(zval).value - decouple_sfr_time / 1e3)
    )
    if sfr_decouple_time_index == 0:
        sfr_decouple_time_index = 2
    mass_lastbins = np.trapezoid(
        x=time_arr_interp[-sfr_decouple_time_index:] * 1e9 * (cosmo.age(zval).value),
        y=sfh[-sfr_decouple_time_index:],
    )
    ...
    if mass_remaining < 0:
        ...
        if print_warnings == True:
            print(
                "input SFR, M* combination is not physically consistent (log M*: %.2f, log SFR: %.2f.)"
                % (sfh_tuple[0], sfh_tuple[1])
            )
    mass_initbins = np.trapezoid(
        x=time_arr_interp[0 : (1000 - sfr_decouple_time_index)]
        * 1e9
        * (cosmo.age(zval).value),
        y=sfh[0 : (1000 - sfr_decouple_time_index)],
    )
    sfh[0 : (1000 - sfr_decouple_time_index)] = (
        sfh[0 : (1000 - sfr_decouple_time_index)] * mass_remaining / mass_initbins
    )

    if (np.abs(np.log10(sfh[-1]) - sfh_tuple[1]) > sfr_tolerance) or (
        decouple_sfr == True
    ):
