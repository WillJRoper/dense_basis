...
def _init_fsps():
    global mocksp
    global _fsps_status
    # Cache the initialization result so repeated calls reuse the same FSPS object.
    if _fsps_status is not None:
        return mocksp
    try:
        import fsps
...
def makespec_atlas(
    atlas, galid, priors, sp, cosmo, filter_list=[], filt_dir=[], return_spec=False
):
    if sp is None:
        sp = get_mocksp()
    ...
    output = makespec(
        specdetails, priors, sp, cosmo, filter_list, filt_dir, return_spec
    )
...
def makespec(
    ...
    if type(return_spec) == type(True):
        ...
        raise (
            "Unknown argument for return_spec. Use True or False, or pass a wavelength grid."
        )
...
def convert_to_splined_spec(spec_peraa, lam, lam_spline, redshift, cosmology=cosmo):
    ...
    spec_ergsec_cm2 = spec_ergsec / (
        4 * np.pi * 3.086e24 * 3.086e24 * lum_dist * lum_dist
    )
...
def make_filvalkit_simple(
    lam, z, fkit_name="filter_list.dat", vb=False, filt_dir="filters/"
):
    # import os
    # print(os.listdir())
    lam_z = (1 + z) * lam
    ...
    for i in range(numlines):
        if (filt_dir == "internal") & (fkit_name == "filter_list_goodss.dat"):
            tempfilt = np.loadtxt(
                get_file("filters/filter_curves/goods_s", temp[i][22:].decode("utf-8"))
            )
        elif (filt_dir == "internal") & (fkit_name == "filter_list_goodsn.dat"):
            tempfilt = np.loadtxt(
                get_file("filters/filter_curves/goods_n", temp[i][22:].decode("utf-8"))
            )
        elif (filt_dir == "internal") & (fkit_name == "filter_list_cosmos.dat"):
            tempfilt = np.loadtxt(
                get_file("filters/filter_curves/cosmos", temp[i][21:].decode("utf-8"))
            )
        elif (filt_dir == "internal") & (fkit_name == "filter_list_egs.dat"):
            tempfilt = np.loadtxt(
                get_file("filters/filter_curves/egs", temp[i][18:].decode("utf-8"))
            )
        elif (filt_dir == "internal") & (fkit_name == "filter_list_uds.dat"):
            tempfilt = np.loadtxt(
                get_file("filters/filter_curves/uds", temp[i][18:].decode("utf-8"))
            )
        else:
            if filt_dir[-1] == "/":
            ...
            if i == numlines - 1:
                ...
                if os.path.exists(filt_name[0:][0:-1]):
                    ...
                    raise Exception(
                        "filters not found. are you sure the folder exists at the right relative path?"
                    )
        ...
        curve_small = np.interp(
            lam_z[bot_in + 1 : top_in - 1], temp_lam_arr, temp_response_curve
        )
    ...
    if vb == True:
        print(
            "created filcurve array splined to input lambda array at redshift: "
            + str(z)
        )
...
def generate_atlas(
    ...
    print(
        priors.Nparam,
        " tx parameters, ",
        priors.sfr_prior_type,
        " SFR sampling",
        priors.sfh_treatment,
        " SFH treatment",
        priors.met_treatment,
        " met sampling",
        priors.dust_model,
        " dust attenuation",
        priors.dust_prior,
    ...
    for i in tqdm(range(int(N_pregrid))):
        ...
        if priors.dynamic_decouple == True:
            priors.decouple_sfr_time = (
                100 * cosmo.age(zval).value / cosmo.age(0.1).value
            )
        ...
        if len(lam_array_spline) > 0:
            ...
            if i == 0:
                ...
                fc_zgrid = np.arange(
                    priors.z_min - z_step, priors.z_max + z_step, z_step
                )
                ...
                for i in range(len(fc_zgrid)):
                    fcs[0:, 0:, i], lzs[0:, i], lzs_lores[0:, i] = (
                        make_filvalkit_simple(
                            lam, fc_zgrid[i], fkit_name=filter_list, filt_dir=filt_dir
                        )
                    )
    ...
    if store == True:
        ...
        if os.path.exists(path):
            ...
            os.mkdir(path)
            print(
                "Created directory and saved atlas at : "
                + path
                + fname
                + "_"
                + str(N_pregrid)
...
def load_atlas(fname, N_pregrid, N_param, path="pregrids/"):
    fname_full = (
        path + fname + "_" + str(N_pregrid) + "_Nparam_" + str(N_param) + ".dbatlas"
    )
