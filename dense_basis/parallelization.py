...
def _require_parallel():
    ...
    if not _has_parallel and not _parallel_warned:
        print("running without parallelization.")
        _parallel_warned = True
    if not _has_parallel:
        raise ImportError("schwimmbad is required for parallelization.")
...
def generate_atlas_in_parallel_chunking(
    ...
    try:
        with MultiPool() as pool:
            values = list(
                pool.map(partial(gen_pg_parallel, atlas_vals=atlas_vals), data)
            )
    finally:
...
def combine_pregrid_chunks(
    ...
    i = 0
    atlas_big = db.load_atlas(
        fname=fname_base + "_chunk_%.0f" % i,
        N_pregrid=chunksize,
        N_param=N_param,
        path=chunk_path,
    )
...
def generate_atlas_in_parallel_zgrid(zgrid, atlas_params, dynamic_decouple=True):
    ...
    try:
        with MultiPool() as pool:
            values = list(
                pool.map(partial(make_atlas_parallel, atlas_params=atlas_params), zgrid)
            )
    finally:
        time_end = time.time()
...
def fit_gals(gal_id, catvals):
    ...
    fit_likelihood, fit_norm_fac = evaluate_sed_likelihood(
        gal_sed, gal_err, atlas, fit_mask=fit_mask, zbest=None, deltaz=None
    )
...
def fit_catalog(
    ...
    for i in range(len(zgrid)):
        ...
        print(
            "starting parallel fitting for Ngals = ",
            len(fit_ids),
            " at redshift ",
            str(zval),
        )
        ...
        try:
            ...
            with MultiPool() as pool:
                ...
                if isinstance(fit_mask, np.ndarray) == False:
                    all_quants = list(
                        pool.map(
                            partial(fit_gals, catvals=(cat_seds, cat_errs, atlas)),
                            fit_ids,
                        )
                    )
                else:
                    all_quants = list(
                        pool.map(
                            partial(
                                fit_gals, catvals=(cat_seds, cat_errs, fit_mask, atlas)
                            ),
                            fit_ids,
                        )
                    )
            ...
            for ii, gal_id in enumerate(fit_ids):
                ...
                if np.isnan(quants[0][0]):
                    ...
                    np.abs(fit_logSFRinst_84[gal_id] - fit_logSFRinst_16[gal_id])
                    > sfr_uncert_cutoff
                ):
        ...
        pl.hist(
            cat_zbest[cat_zbest > 0], np.arange(0, 6, z_bw), color="black", alpha=0.3
        )
