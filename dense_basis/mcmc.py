...
def _require_emcee():
    global _emcee_warned
    if not _has_emcee and not _emcee_warned:
        print("running without emcee")
        _emcee_warned = True
    if not _has_emcee:
        raise ImportError("emcee is required to run MCMC sampling.")


def get_mcmc_variables(atlas):
    model_params = np.vstack(
        (
            atlas["mstar"],
            atlas["sfr"],
...
def run_emceesampler(
    ...
    _, norm_fac = evaluate_sed_likelihood(
        gal_sed, gal_err, atlas, fit_mask=[], zbest=None, deltaz=None
    )
